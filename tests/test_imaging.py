...
class TestImaging:
    ...
    tl_third = np.array([0, 0, 0, 1, 1, 2, 3, 5, 8, 13, 21, 32, 41, 47, 46, 44, 58, 77, 61, 75, 56, 54])
    title = 'Title'
    label = 'Label'
    ...
    def test_plot_octave_kHz(self):
        plot_octave(self.tl_oct, self.octaves, kHz=True, xlabel=self.label, ylabel=self.label, title=self.title, separator='.')
    ...
    def test_plot_third_octave_kHz(self):
        plot_third(self.tl_third, self.thirds, marker='s', kHz=True, xlabel=self.label, ylabel=self.label, title=self.title)
